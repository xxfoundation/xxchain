...
impl<AccountId: Ord + MaxEncodedLen> Default for EraDeductionPoints<AccountId> {
    fn default() -> Self {
        Self {
            total: 0,
            individual: BoundedBTreeMap::new(),
        }
    }
}
...
pub mod pallet {
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn set_cmix_id(
            ...
            // Ensure the account is bonded (has a ledger)
                ...
                pallet_staking::Ledger::<T>::contains_key(&stash),
                Error::<T>::NotBonded
            );

            // Set the cmix_id
            CmixIds::<T>::insert(&stash, cmix_id);
            Self::deposit_event(Event::CmixIdSet {
                stash,
                cmix_id,
            });
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn deduct_by_ids(validators_points: impl IntoIterator<Item = (T::AccountId, u32)>) {
            ...
            ErasDeductionPoints::<T>::mutate(active_era, |era_deductions| {
                ...
                for (validator, points) in validators_points {
                    ...
                    let current_points = era_deductions.individual
                        .get(&validator)
                        .copied()
                        .unwrap_or(0);
                    let _ = era_deductions.individual
                        .try_insert(validator, current_points.saturating_add(points));
                }

                if total_deducted > 0 {
                    Self::deposit_event(Event::PointsDeducted {
                        era: active_era,
                        total_deducted,
                    });
                }
        ...
        pub fn do_clear_cmix_id(stash: &T::AccountId) {
            CmixIds::<T>::remove(stash);
            Self::deposit_event(Event::CmixIdCleared {
                stash: stash.clone(),
            });
        }
