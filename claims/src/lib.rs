...
use frame_system::{ensure_none, ensure_root, ensure_signed, pallet_prelude::BlockNumberFor};
use scale_info::TypeInfo;
use serde::{self, Deserialize, Serialize};
use sp_io::{crypto::secp256k1_ecdsa_recover, hashing::keccak_256};
use sp_runtime::{
	impl_tx_ext_default,
	traits::{CheckedSub, DispatchInfoOf, Dispatchable, TransactionExtension},
...
impl StatementKind {
	...
	fn to_text(self) -> &'static [u8] {
		match self {
			...
			StatementKind::Saft => &b"xx network doesn't have a SAFT statement"[..],
		}
	}
}
...
pub mod pallet {
	...
	pub type Claims<T: Config> =
		StorageMap<_, Identity, EthereumAddress, BalanceOf<T>, OptionQuery>;
	...
	pub type Signing<T: Config> =
		StorageMap<_, Identity, EthereumAddress, StatementKind, OptionQuery>;

	/// Pre-claimed Ethereum accounts, by the Account ID that they are claimed to.
	#[pallet::storage]
	#[pallet::getter(fn preclaims)]
	pub type Preclaims<T: Config> =
		StorageMap<_, Identity, T::AccountId, EthereumAddress, OptionQuery>;
	...
	impl<T: Config> BuildGenesisConfig for GenesisConfig<T> {
		fn build(&self) {
			...
			// Build total
			let total =
				self.claims.iter().fold(BalanceOf::<T>::default(), |acc, (_, b, _, _, _)| acc + *b);
			Total::<T>::put(total);

			// Build signing
			self.claims.iter().filter_map(|(a, _, _, _, s)| Some((*a, (*s)?))).for_each(
				|(a, s)| {
			...
			// Build preclaims
			self.claims.iter().filter_map(|(a, _, _, i, _)| Some((i.clone()?, *a))).for_each(
				|(i, a)| {
			...
			// Build rewards
			self.claims.iter().filter_map(|(a, _, c, _, _)| Some((*a, (*c)?))).for_each(
				|(a, r)| {
...
impl<T: Config> Pallet<T> {
	...
	fn process_claim(signer: EthereumAddress, dest: T::AccountId) -> DispatchResult {
		...
		if let Some(rw) = reward {
			T::RewardHandler::add_claimed(dest.clone(), balance_due, rw)
		}
...
pub struct PrevalidateAttests<T: Config + Send + Sync>(core::marker::PhantomData<T>)
where
...
where
	<T as frame_system::Config>::RuntimeCall:
		IsSubType<Call<T>> + Dispatchable<RuntimeOrigin = T::RuntimeOrigin>,
...
mod secp_utils {
	...
	pub fn sig<T: Config>(
		...
		let msg =
			keccak_256(&<Pallet<T>>::ethereum_signable_message(&to_ascii_hex(what)[..], extra));
...
mod tests {
	...
	use pallet_balances;
	use sp_runtime::{
		traits::{BlakeTwo256, Identity, IdentityLookup, TxBaseImplication},
	...
	pub fn new_test_ext() -> sp_io::TestExternalities {
		...
		claims::GenesisConfig::<Test> {
			...
			vesting: vec![
				...
				(eth(&charlie()), vec![(50, 10, 0), (450, 10, 0)]),
			],
		}
	...
	fn claiming_with_too_many_schedules_ignores_excess() {
		new_test_ext().execute_with(|| {
			...
			assert_ok!(Claims::mint_claim(
				...
				eth(&bob()),
				200,
				Some(vec![(50, 10, 1), (50, 10, 1), (50, 10, 1), (50, 10, 1),]),
				None
			));
			// New total
			assert_eq!(Claims::total(), total_claims() + 200);
