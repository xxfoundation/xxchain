...
fn set_testnet_manager_account_called_by_admin_works() {
	ExtBuilder::default().build_and_execute(|| {
		assert_ok!(XXPublic::set_testnet_manager_account(
			RuntimeOrigin::signed(AdminAccount::get()),
			45
		));
		assert_eq!(XXPublic::testnet_manager().unwrap(), 45);
		assert_eq!(xx_public_events(), vec![super::Event::TestnetManagerUpdated]);
	})
}
...
fn set_sale_manager_account_called_by_non_admin_fails() {
	ExtBuilder::default().build_and_execute(|| {
		assert_noop!(XXPublic::set_sale_manager_account(RuntimeOrigin::signed(1), 45), BadOrigin);
	})
}
...
fn set_sale_manager_account_called_by_admin_works() {
	ExtBuilder::default().build_and_execute(|| {
		assert_ok!(XXPublic::set_sale_manager_account(
			RuntimeOrigin::signed(AdminAccount::get()),
			46
		));
		assert_eq!(XXPublic::sale_manager().unwrap(), 46);
		assert_eq!(xx_public_events(), vec![super::Event::SaleManagerUpdated]);
	})
}
...
fn testnet_distribute_called_by_manager_works() {
	ExtBuilder::default().with_testnet_balance(1000).build_and_execute(|| {
		assert_ok!(XXPublic::testnet_distribute(
			RuntimeOrigin::signed(42),
			vec![TransferData::<AccountId, Balance, u64> {
				destination: 10,
				amount: 100,
				schedules: None
			}],
...
fn sale_distribute_called_by_manager_works() {
	ExtBuilder::default().with_sale_balance(1000).build_and_execute(|| {
		assert_ok!(XXPublic::sale_distribute(
...
fn distributions_fail_when_not_enough_funds() {
	ExtBuilder::default().with_testnet_balance(1000).with_sale_balance(1000).build_and_execute(
		|| {
			assert_noop!(
...
fn distribute_to_account_with_existing_vesting() {
	ExtBuilder::default().with_sale_balance(1000).with_vesting().build_and_execute(|| {
		assert_ok!(XXPublic::sale_distribute(
			RuntimeOrigin::signed(43),
			vec![
				TransferData::<AccountId, Balance, u64> {
