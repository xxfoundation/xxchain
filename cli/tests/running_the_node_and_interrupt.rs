...
async fn running_the_node_works_and_can_be_interrupted() {
	async fn run_command_and_kill(signal: Signal) {
		...
		let mut cmd = common::KillChildOnDrop(
			Command::new(cargo_bin("xxnetwork-chain"))
		...
		common::wait_n_finalized_blocks(3, 60, &ws_url).await.expect("Blocks are produced in time");
		...
		let stats_file = base_path.path().join("chains/xxnetwork_dev/paritydb/full/stats.txt");
