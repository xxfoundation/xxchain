...
use frame_support::{
	...
	traits::{
		ConstU32, EqualPrivilegeOnly, FindAuthor, Imbalance, InstanceFilter, LockIdentifier,
...
impl OnUnbalanced<frame_support::traits::fungible::Credit<AccountId, Balances>>
	for RewardRemainderMock
{
	...
	fn filter(&self, c: &RuntimeCall) -> bool {
		match self {
			...
			ProxyType::Governance => {
				matches!(c, RuntimeCall::Democracy(..) | RuntimeCall::Elections(..))
			}
...
pub(crate) fn xx_team_custody_events() -> Vec<xx_team_custody::Event<Test>> {
	System::events()
		.into_iter()
		.map(|r| r.event)
		.filter_map(|e| if let RuntimeEvent::XXCustody(inner) = e { Some(inner) } else { None })
		.collect()
}
...
pub(crate) fn proxy_events() -> Vec<pallet_proxy::Event<Test>> {
	System::events()
		.into_iter()
		.map(|r| r.event)
		.filter_map(|e| if let RuntimeEvent::Proxy(inner) = e { Some(inner) } else { None })
		.collect()
}
