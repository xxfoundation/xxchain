...
use crate::{BalanceOf, Config, CustodyAccounts, Error, Event, Pallet, TeamAccounts, TotalCustody};
...
use frame_support::{
	...
	traits::{
		fungible::Inspect, Currency, ExistenceRequirement::AllowDeath, Get, ReservableCurrency,
	},
};
use scale_info::TypeInfo;
use sp_runtime::traits::{AtLeast32BitUnsigned, Convert, SaturatedConversion, Saturating, Zero};
...
pub struct CustodyInfo<
	AccountId,
	Balance: HasCompact + frame_support::pallet_prelude::MaxEncodedLen,
> {
	/// Allocation
	#[codec(compact)]
...
impl<AccountId, Balance> CustodyInfo<AccountId, Balance>
where
...
pub fn initialize_custody<T: Config>(who: &T::AccountId, amount: BalanceOf<T>) {
	...
	let custody_account =
		<pallet_proxy::Pallet<T>>::pure_account(who, &T::CustodyProxy::get(), 0, None);

	// Deposit amount in custody account
	// The imbalance must be dropped to update TotalIssuance
	let imbalance = <T as Config>::Currency::deposit_creating(&custody_account, custody_balance);
	drop(imbalance);
	...
	// the anonymous account function from proxy pallet
	let reserve_account =
		<pallet_proxy::Pallet<T>>::pure_account(who, &T::CustodyProxy::get(), 1, None);

	// Deposit amount in reserve account
	// The imbalance must be dropped to update TotalIssuance
	let imbalance = <T as Config>::Currency::deposit_creating(&reserve_account, reserve_balance);
	drop(imbalance);
...
fn set_custody_governance_proxy<T: Config>(
	custody: &T::AccountId,
	proxy: T::AccountId,
) -> DispatchResult {
...
fn remove_custody_proxies<T: Config>(custody: &T::AccountId) {
	...
	<T as pallet_proxy::Config>::Currency::unreserve(custody, old_deposit);
}

/// Compute payout
fn compute_payout<T: Config>(allocation: BalanceOf<T>) -> BalanceOf<T> {
	let payout_ratio = Perbill::from_rational(payout_frequency::<T>(), custody_duration::<T>());
...
fn do_payout<T: Config>(
	...
	let custody_transferable_balance = <T as Config>::Currency::reducible_balance(
		&custody,
		preservation,
	...
	let custody_balance =
		<BalanceOf<T>>::try_from(custody_transferable_balance.saturated_into::<u128>())
			.ok()
			.unwrap_or(Zero::zero());
	...
	let reserve_transferable_balance = <T as Config>::Currency::reducible_balance(
		&reserve,
		preservation,
		frame_support::traits::tokens::Fortitude::Polite,
	);
	...
	// balance type is the same. However, explicit conversion is needed here.
	let reserve_balance =
		<BalanceOf<T>>::try_from(reserve_transferable_balance.saturated_into::<u128>())
			.ok()
			.unwrap_or(Zero::zero());
	...
	if !withdraw_custody.is_zero() {
		...
		<T as Config>::Currency::transfer(&custody, &who, withdraw_custody, AllowDeath)?;
		// Emit TeamPayoutCustody event
		Pallet::<T>::deposit_event(Event::PayoutFromCustody {
			who: who.clone(),
			amount: withdraw_custody,
		});
	}
	...
	if !withdraw_reserve.is_zero() {
		...
		<T as Config>::Currency::transfer(&reserve, &who, withdraw_reserve, AllowDeath)?;
		// Emit TeamPayoutReserve event
		Pallet::<T>::deposit_event(Event::PayoutFromReserve {
			who: who.clone(),
			amount: withdraw_reserve,
		});
	}
	...
	TotalCustody::<T>::mutate(|n| *n = (*n).saturating_sub(custody_deduct));
...
pub fn try_custody_bond_extra<T: Config>(
	...
	<pallet_staking::Pallet<T>>::bond_extra(T::RuntimeOrigin::from(Some(custody).into()), value)
}
...
pub fn try_team_custody_set_proxy<T: Config>(
	who: T::AccountId,
	proxy: T::AccountId,
) -> DispatchResult {
